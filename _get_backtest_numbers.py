...
df_account_value_a2c, _ = DRLAgent.DRL_prediction(model=trained_a2c, environment=e_trade_gym)
df_account_value_ddpg, _ = DRLAgent.DRL_prediction(model=trained_ddpg, environment=e_trade_gym)
df_account_value_ppo, _ = DRLAgent.DRL_prediction(model=trained_ppo, environment=e_trade_gym)
df_account_value_td3, _ = DRLAgent.DRL_prediction(model=trained_td3, environment=e_trade_gym)
df_account_value_sac, _ = DRLAgent.DRL_prediction(model=trained_sac, environment=e_trade_gym)
...
result = pd.DataFrame({
    "a2c": df_account_value_a2c.set_index(df_account_value_a2c.columns[0])["account_value"],
    "ddpg": df_account_value_ddpg.set_index(df_account_value_ddpg.columns[0])["account_value"],
    "ppo": df_account_value_ppo.set_index(df_account_value_ppo.columns[0])["account_value"],
    "td3": df_account_value_td3.set_index(df_account_value_td3.columns[0])["account_value"],
    "sac": df_account_value_sac.set_index(df_account_value_sac.columns[0])["account_value"],
...
print(((final_row - initial) / initial * 100).round(2))
